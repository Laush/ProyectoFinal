...
namespace API
{
    public class ImagenesUtility
    {
        ...
        public static string Guardar(HttpPostedFileBase archivoSubido, string nombreSignificativo)
        {
            ...
            string carpetaImagenes = System.Configuration.ConfigurationManager.AppSettings["FotosDePerfil"];

            if (string.IsNullOrEmpty(carpetaImagenes))
            {
                throw new Exception("En el archivo web.config debe agregar dentro de <appSettings> el elemento <add key=\"FotosDePerfil\" value=\"/FotosDePerfil/\" />");
            }
